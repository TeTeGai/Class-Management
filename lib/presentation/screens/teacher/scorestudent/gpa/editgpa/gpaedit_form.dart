...
class GpaEditForm extends StatefulWidget {
  final String GoodOrBad;
  final GPAModel gpaModel;
  const GpaEditForm({Key? key, required this.GoodOrBad, required this.gpaModel}) : super(key: key);

  @override
  State<GpaEditForm> createState() => _GpaAddFormState();
}

class _GpaAddFormState extends State<GpaEditForm> {
  ...
  void onPress()
  {
    ...
    if(scoreEditingController.text.trim().isNotEmpty && nameEditingController.text.trim().isNotEmpty)
    {
      ...
      if(widget.GoodOrBad == "Good")
      {
        ...
        try{
          scoreBloc.add(GPAModelUpdate(gpaModel));
