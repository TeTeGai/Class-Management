...
class GpaSonChartScreen extends StatefulWidget {
  final StudentModel studentModel;
  const GpaSonChartScreen({Key? key, required this.studentModel}) : super(key: key);

  @override
  State<GpaSonChartScreen> createState() => _GpaSonChartScreenState();
}

class _GpaSonChartScreenState extends State<GpaSonChartScreen> {
  ...
  Future<int> getGoodGpa() async {
    ...
    var respectsQuery =     FirebaseFirestore.instance
        .collection("class").doc(widget.studentModel.classId).collection("gpaList").where("idStudent",isEqualTo: widget.studentModel.idStudent).where("category",isEqualTo: "GoodGPA");
  ...
  Future<int> getBadGpa() async {
    ...
    var respectsQuery =  FirebaseFirestore.instance
        .collection("class").doc(widget.studentModel.classId).collection("gpaList").where("idStudent",isEqualTo: widget.studentModel.idStudent).where("category",isEqualTo: "BadGPA");
    var querySnapshot = await respectsQuery.get();
    var totalEquals = querySnapshot.docs.length;
    return totalEquals;
  }
