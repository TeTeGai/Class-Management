...
class EditSonGpaGoodForm extends StatefulWidget {

  const EditSonGpaGoodForm({Key? key,}) : super(key: key);

  @override
  State<EditSonGpaGoodForm> createState() => _EditSonGpaGoodFormState();
}

class _EditSonGpaGoodFormState extends State<EditSonGpaGoodForm> {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title:  ConstrainedBox(
            ...
            child: Text('Chỉnh sửa điểm tích cực',style: TextStyle(fontSize: 18.sp,fontWeight: FontWeight.bold),)
        ),
        centerTitle: true,
        leading: Padding(
      ...
      body: BlocBuilder<ScoreBloc,ScoreState>(
        ...
        buildWhen: (previous, current) => current is GPAGoodSonLoaded,
              ...
              if(state is GPAGoodSonLoaded)
              {
                ...
                return Stack(
                  children: [
                    gpamodel.length>0 ? AnimationLimiter(
                      child: GridView.builder(
                        ...
                        itemBuilder: (context, index) {
                          if (index == 0)
                            {
                             return GPASonAddWidget(GoodOrBad: "Good",);
                            }
                                  ...
                                  child: FadeInAnimation(
                                    child: Container(
                                      child: Padding(
                                          ...
                                          child: TextButton(
                                              onPressed: (){
                                                List lis = ['Good',gpamodel[index-1]];
                                                 Navigator.of(context).pushNamed(AppRouter.EDITSONGPA,arguments:lis );
                                                },
                                              child:  Container(
                                                child: Stack(
                                                  children: [
                    ...
                    ) :GPASonAddWidget(GoodOrBad: "Good",)
                  ],
                );
              }
