...
import 'package:luan_an/presentation/screens/parent/news/sons/soninfor/editgpason/gpason_editwidget.dart';
...
class GpaGoodSonForm extends StatefulWidget {
  final StudentModel studentModel;
  const GpaGoodSonForm({Key? key, required this.studentModel,}) : super(key: key);

  @override
  State<GpaGoodSonForm> createState() => _GpaGoodSonFormState();
}

class _GpaGoodSonFormState extends State<GpaGoodSonForm> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ScoreBloc,ScoreState>(
      bloc: ScoreBloc()..add(GPAGoodSonLoad()),
          builder: (context, state) {
            if (state is GPALoading)
            {
              Text("Sai");
            }
            if(state is GPAGoodSonLoaded)
            {
              ...
              return Stack(
                children: [
                  gpamodel.length>0 ? AnimationLimiter(
                    child: GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        if (index == 0)
                          {
                           return GPASonEditWidget(GoodOrBad: "Good",isConnect: widget.studentModel.isConnect,);
                          }
                                  ...
                                  child: TextButton(
                                      onPressed: (){
                                        if(widget.studentModel.isConnect)
                                        {
                                          ...
                                          MySnackBar.error(message: widget.studentModel.name+" đã được cộng "+ gpamodel[index - 1].score.toString() + " điểm do "+gpamodel[index - 1].title, color: Colors.cyan, context: context);
                  ...
                  ) :GPASonEditWidget(GoodOrBad: "Good",isConnect: widget.studentModel.isConnect,)
                ],
              );
            }
